...
%%Question for Todd: How do I make this function stop giving me all the f
%%and z values? Can I suppress it somehow so I can just see x and y????
...
x(1)= GenerateRandomBinaryOutcome_Sversion(pi);
...
for t=2:n;
    ...
    %The two steps above tell the RandomOutcome function which row of the Q
    %matrix to consider when evaluating how z is mapped to X (the state).  
    %So the function:GenerateRandomBinaryOutcome_Sversion is like the S(z) function, 
    ...
    x(t)=GenerateRandomBinaryOutcome_Sversion(currentPMFx);
    xcurrent=x(t);
...
for t=1:n;
    ...
    y(t)=GenerateRandomBinaryOutcome_Sversion(currentPMFy);
end
x;
y;
